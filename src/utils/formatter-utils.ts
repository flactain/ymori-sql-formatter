...
export class FormatterUtils {
  ...
  static calculateGlobalMaxKeywordLength(stmt: any): number {
    ...
    return allKeywords.length > 0
      ? Math.max(...allKeywords.map((k) => k.length))
      : 0;
  }
  ...
  /**
   * ヒント句を抽出（ダミーヒント句自動挿入機能付き）
   */
  static extractHintComment(
    sql: string,
    insertDummyHint: boolean = false
  ): { sql: string; hint: string | null } {
    ...
    // ヒント句が存在しない場合、ダミーヒント句を挿入
    if (insertDummyHint) {
      const dummyHint = this.generateDummyHint();
      return {
        sql: sql,
        hint: dummyHint,
      };
   ...
   * ダミーヒント句を生成
   */
  static generateDummyHint(): string {
    const now = new Date();
    const timestamp = now
   ...
   * 未知の式を処理（拡張可能な設計）
   */
  static handleUnknownExpression(
    expr: any,
    _depth: number = 0,
    _maxKeywordLength?: number
  ): string {
